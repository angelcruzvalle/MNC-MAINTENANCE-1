...
function locationNameForId(state={}, id="") {
  if(!id || id === "__all") return "All Locations";
...
function reducer(state, { type, payload }) {
  ...
  switch(type) {
    ...
    case "ADD_LOCATION": {
      ...
      const loc = { id: payload?.id || `LOC-${Date.now()}`, name, address:payload?.address||"", manager:payload?.manager||"", active:true };
...
function SystemSettings({ state, dispatch, onClose }) {
  ...
  const [inviteForm, setInviteForm] = useState({ email:"", name:"", role:"Location Administrator", locationId:orgLocations[0]?.id || "" });
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:0, maxHeight:"70vh", overflowY:"auto" }}>
        ...
        <div style={{ marginBottom:16 }}>
          ...
          <div style={{ marginTop:14, padding:12, border:`1px solid ${T.border}`, borderRadius:8, background:T.grayLt }}>
            <div style={{ fontFamily:T.sans, fontSize:12, fontWeight:800, color:T.text, marginBottom:8 }}>Invite User to a Location</div>
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8 }}>
              ...
              <select style={sel} value={inviteForm.role} onChange={e=>setInviteForm(f=>({...f,role:e.target.value}))}><option>Location Administrator</option><option>Mechanic</option><option>Read Only</option></select>
            ...
            <div style={{ display:"flex", justifyContent:"space-between", gap:8, marginTop:8, alignItems:"center" }}>
              ...
              <Btn small onClick={()=>{ if(!inviteForm.email.trim()) return alert("Enter an email first."); if(!inviteForm.locationId) return alert("Create/select a location first."); const payload=createInvitePayload(); dispatch({type:"ADD_USER_INVITE", payload}); alert("Invite created. Send the invite link to the user so they can create their account for only this location."); setInviteForm(f=>({...f,email:"",name:""})); }}>Create Invite</Btn>
            </div>
          ...
          <div style={{ marginTop:14, padding:12, border:`1px solid ${T.border}`, borderRadius:8, background:T.grayLt }}>
            <div style={{ fontFamily:T.sans, fontSize:12, fontWeight:800, color:T.text, marginBottom:8 }}>Migrate / Copy Templates Between Locations</div>
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8 }}>
              <Field label="From Location" half><select style={sel} value={migrationForm.fromId} onChange={e=>setMigrationForm(f=>({...f,fromId:e.target.value}))}>{orgLocations.map(l=><option key={l.id} value={l.id}>{l.name}</option>)}</select></Field>
              <Field label="To Location" half><select style={sel} value={migrationForm.toId} onChange={e=>setMigrationForm(f=>({...f,toId:e.target.value}))}>{orgLocations.map(l=><option key={l.id} value={l.id}>{l.name}</option>)}</select></Field>
            </div>
            ...
            <div style={{ display:"flex", justifyContent:"space-between", gap:8, marginTop:8, alignItems:"center" }}>
              <div style={{ fontSize:11, color:T.muted }}>This creates independent copies in the destination location. The destination mechanic can edit their copy without changing the original templates.</div>
              <Btn small onClick={()=>{ if(migrationForm.fromId===migrationForm.toId) return alert("Choose two different locations."); dispatch({type:"MIGRATE_TEMPLATES", payload:migrationForm}); alert("Independent copies created in destination location."); }}>Copy Selected</Btn>
            </div>
          </div>
    ...
    <div data-theme={effectiveTheme} style={{ minHeight:"100vh", background:T.bg, color:T.text, fontFamily:T.sans }}>
      ...
      <header className="no-print" style={{ position:"sticky", top:0, zIndex:1000, background:T.surface, borderBottom:`1px solid ${T.border}`, padding:"0 20px", height:56, display:"flex", alignItems:"center", justifyContent:"space-between", boxShadow:`0 1px 0 ${T.border}` }}>
        <div style={{ display:"flex", alignItems:"center", gap:14 }}>
          ...
          <select title="Switch location" value={state.activeLocationId || "__all"} onChange={e=>dispatch({type:"SET_ACTIVE_LOCATION", payload:e.target.value})} style={{ ...sel, width:190, padding:"5px 9px", fontSize:12 }}>
            <option value="__all">Organization Dashboard / All Locations</option>
            {maintLocations.map(l=><option key={l.id} value={l.id}>{l.name}</option>)}
          </select>
          <span style={{ fontFamily:T.sans, fontSize:11, color:T.muted }}>Viewing: {activeLocationLabel}</span>
        </div>
        <div style={{ display:"flex", alignItems:"center", gap:10 }}>
