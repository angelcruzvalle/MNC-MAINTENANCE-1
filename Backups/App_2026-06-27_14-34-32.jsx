...
function normalizeLoadedUserState(data={}, ownerUserId="") {
  ...
  return autoAssignLegacyDataToDefaultFacility(merged);
}
...
function recordMatchesActiveLocation(record={}, state={}) {
  const loc = activeMaintForgeLocation(state);
  if(!loc) return true;
  if(recordFacilityId(record) === loc.id) return true;
  ...
  // It is automatically assigned to Morovis National Cemetery when that facility exists.
  return false;
}

function isLegacyUnassignedRecord(record={}) {
  return !recordFacilityId(record) && !recordLocationName(record, {}).trim();
}
