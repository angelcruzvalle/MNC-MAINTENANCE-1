...
function SystemSettings({ state, dispatch, onClose }) {
  ...
  const FacilityInput = ({ label, value, onChange, placeholder="" }) => (
    <div style={{ marginBottom:10 }}>
      <label style={{ display:"block", fontFamily:T.sans, fontSize:12, fontWeight:700, color:T.subtext, marginBottom:5 }}>{label}</label>
      <input style={{ ...inp, width:"100%", boxSizing:"border-box" }} value={value || ""} onChange={onChange} placeholder={placeholder} />
    </div>
  );
            ...
            <div style={{ display:"grid", gridTemplateColumns:"260px 1fr", gap:12, alignItems:"start" }}>
              ...
              <div style={{ padding:12, border:`1px solid ${T.border}`, borderRadius:8, background:T.surface }}>
                ...
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"0 12px" }}>
                  <div style={{ gridColumn:"span 2" }}><FacilityInput label="Facility Name" value={selectedFacility?.name||""} onChange={e=>updateSelectedFacility({ name:e.target.value })} /></div>
                  <FacilityInput label="Manager" value={selectedFacility?.manager||""} onChange={e=>updateSelectedFacility({ manager:e.target.value })} />
                  <div style={{ gridColumn:"span 2" }}><FacilityInput label="Address" value={selectedFacility?.address||""} onChange={e=>updateSelectedFacility({ address:e.target.value })} /></div>
                  <FacilityInput label="City, State" value={selectedFacility?.cityState||""} onChange={e=>updateSelectedFacility({ cityState:e.target.value })} />
                  <FacilityInput label="Phone" value={selectedFacility?.phone||""} onChange={e=>updateSelectedFacility({ phone:e.target.value })} />
                  <FacilityInput label="Email" value={selectedFacility?.email||""} onChange={e=>updateSelectedFacility({ email:e.target.value })} />
                </div>
