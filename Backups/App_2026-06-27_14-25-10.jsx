...
function recordMatchesActiveLocation(record={}, state={}) {
  const loc = activeMaintForgeLocation(state);
  if(!loc) return true;
  if(record.locationId === loc.id) return true;
  const recName = recordLocationName(record, state).toLowerCase();
  if(recName && recName === loc.name.toLowerCase()) return true;
  // Legacy data from older single-facility versions may have no facility stamp.
  // The owner can assign that data from Organization view or the active facility repair banner.
  return false;
}

function isLegacyUnassignedRecord(record={}) {
  return !record?.locationId && !recordLocationName(record, {}).trim();
}
...
function stampLegacyList(list=[], facilityId='', facilityName='') {
  return (list || []).map(item => isLegacyUnassignedRecord(item) ? { ...item, locationId:facilityId, locationName:facilityName, location:facilityName } : item);
}
...
function migrationSourceItems(list=[], state={}, fromId="") {
  ...
  return (list||[]).filter(item => !item?.locationId && !recordLocationName(item, state));
}
...
function reducer(state, { type, payload }) {
  ...
  switch(type) {
    ...
    case "ASSIGN_LEGACY_TO_FACILITY": {
      ...
      const stamp = list => stampLegacyList(list, facilityId, facilityName);
      return {
        ...state,
    ...
    case "UPDATE_SETTINGS": {
      ...
      return { ...state, settings: { ...payload, locations:normalizedLocations }, locations: normalizedLocations };
    }
    case "COMPLETE_SETUP": {
            ...
            <div style={{ display:"grid", gridTemplateColumns:"260px 1fr", gap:12, alignItems:"start" }}>
              ...
              <div style={{ padding:12, border:`1px solid ${T.border}`, borderRadius:8, background:T.surface }}>
                ...
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"0 12px" }}>
                  <Field label="Facility Name"><input style={inp} value={selectedFacility?.name||""} onChange={e=>updateSelectedFacility({ name:e.target.value })} /></Field>
                  <Field label="Manager" half><input style={inp} value={selectedFacility?.manager||""} onChange={e=>updateSelectedFacility({ manager:e.target.value })} /></Field>
                  <Field label="Address"><input style={inp} value={selectedFacility?.address||""} onChange={e=>updateSelectedFacility({ address:e.target.value })} /></Field>
                  <Field label="City, State" half><input style={inp} value={selectedFacility?.cityState||""} onChange={e=>updateSelectedFacility({ cityState:e.target.value })} /></Field>
                  <Field label="Phone" half><input style={inp} value={selectedFacility?.phone||""} onChange={e=>updateSelectedFacility({ phone:e.target.value })} /></Field>
                  <Field label="Email" half><input style={inp} value={selectedFacility?.email||""} onChange={e=>updateSelectedFacility({ email:e.target.value })} /></Field>
                </div>
                <button onClick={()=>setForm(f=>{ const list=facilityListForForm(); const removeId=f._selectedFacilityId; const next=list.filter(l=>l.id!==removeId); return {...f,locations:next,_selectedFacilityId:next[0]?.id||""}; })} style={{ background:"none", border:"none", color:T.red, cursor:"pointer", fontFamily:T.sans, fontSize:12, fontWeight:700, padding:0 }}>Delete selected facility</button>
              </div>
            </div>
          )}
