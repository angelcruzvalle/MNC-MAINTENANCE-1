...
function Dashboard({ state, setTab }) {
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
      ...
      {/* Shop Health Banner */}
      <div style={{ background:issues===0?"#f0fdf4":"#fff", border:`2px solid ${healthColor}`, borderRadius:10, padding:"14px 20px", display:"flex", alignItems:"center", gap:14 }}>
        <div style={{ fontSize:32 }}>{issues===0?"✅":issues<=3?"⚠️":"🚨"}</div>
        <div style={{ flex:1 }}>
          <div style={{ fontFamily:T.sans, fontSize:16, fontWeight:800, color:healthColor }}>{health}</div>
          <div style={{ fontFamily:T.sans, fontSize:12, color:T.muted, marginTop:2 }}>
            {issues===0
              ? "No outstanding issues — shop running smoothly."
              : `${outOfSvc>0?`${outOfSvc} OOS equipment · `:""}`+
                `${pmOverdue>0?`${pmOverdue} PM overdue · `:""}`+
        ...
        <div style={{ textAlign:"right" }}>
          <div style={{ fontFamily:T.sans, fontSize:10, color:T.muted, textTransform:"uppercase", letterSpacing:.5 }}>This Month</div>
          <div style={{ fontFamily:T.sans, fontSize:20, fontWeight:700, color:T.accent }}>${spendMo.toFixed(0)}</div>
...
function WorkOrders({ state, dispatch, woSettings, onWOSettings }) {
  ...
  const filtered = state.workOrders.filter(w=>{
    const matchStatus   = filter==="Active"?(w.status==="Open"||w.status==="In Progress"):filter==="All"?true:w.status===filter;
    ...
    return matchStatus && matchType && matchPriority && matchMech;
  }).sort((a,b)=>{
    let cmp = 0;
  ...
  const openDetail = (wo) => { setDetailWO(wo); setForm({...wo, partsUsed:wo.partsUsed||[]}); setModal("detail"); };
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
        ...
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", flexWrap:"wrap", gap:10 }}>
          ...
          <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
            <Btn small onClick={()=>printWO(wo)}>Print</Btn>
            {!isCompleted && <Btn small onClick={updateWO} style={{ background:"#1e40af", borderColor:"#1e40af" }}>Update</Btn>}
            {!isCompleted && <Btn small onClick={completeWO} style={{ background:T.green, borderColor:T.green }}>Complete WO</Btn>}
  ...
  return (
    <div>
      ...
      {/* Detail modal — full editable form */}
      {modal==="detail" && detailWO && (
        <Modal title={`Work Order — ${detailWO.id}`} onClose={()=>{ setModal(null); setDetailWO(null); }}>
          ...
          <div style={{ borderTop:`2px solid ${T.border}`, marginTop:16, paddingTop:16 }}>
            <div style={{ fontFamily:T.sans, fontSize:12, fontWeight:700, color:T.muted, textTransform:"uppercase", letterSpacing:.5, marginBottom:12 }}>Edit Work Order</div>
            {renderWOForm()}
          </div>
        </Modal>
      )}
...
function EquipmentInventory({ state, dispatch }) {
  ...
  const saveTurnIn = () => {
    ...
    if(!item) return;
    dispatch({type:"UPDATE_INV", payload:{...item, turnInStatus:"Pending Turn-in", turnInReason:turnInForm.reason, turnInDate:turnInForm.date, turnInPaperwork:turnInForm.paperwork}});
    setModal(null);
  };
  ...
  const del = id => { if(confirm("Permanently delete this inventory item?")) dispatch({type:"DELETE_INV",payload:id}); };
...
export default function App() {
  const [state, dispatch]   = useReducer(reducer, { ...INIT, inventoryItems:[], profile:null, woSettings:null });
