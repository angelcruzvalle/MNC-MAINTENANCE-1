...
function reducer(state, { type, payload }) {
  switch(type) {
    ...
    case "RESTOCK_PARTS": {
      let parts = [...state.parts];
      (payload||[]).forEach(({partId,qty})=>{ parts = parts.map(p => p.id===partId ? {...p,qty:(+(p.qty||0))+(+qty||0)} : p); });
      return { ...state, parts };
    }
    ...
    case "ADD_USAGE_LOG": {
      const newLogs = [...(state.usageLogs||[]), payload];
      /* Check all schedules (PM + inspection) for this equipment for usage-based triggers */
      const newWOs = [];
      const allLogsForEq = newLogs.filter(l => l.equipmentId === payload.equipmentId);
      const currentHours  = Math.max(...allLogsForEq.map(l => +(l.hours||0)), 0);
      const currentMiles  = Math.max(...allLogsForEq.map(l => +(l.mileage||0)), 0);

      const checkAndCreateWO = (sch, isInspection) => {
        if (sch.equipmentId !== payload.equipmentId) return null;
        if (sch.triggerType === "time") return null;
        const usage = sch.usageType === "mileage" ? currentMiles : currentHours;
        if (!sch.nextDueUsage || usage < +sch.nextDueUsage) return null;
        /* Skip if there's already an open WO for this schedule */
        const alreadyOpen = [...state.workOrders, ...newWOs].some(w =>
          w.equipment === sch.equipmentId &&
          w.scheduleId === sch.id &&
          (w.status === "Open" || w.status === "In Progress")
        );
        if (alreadyOpen) return null;
        const existing = [...state.workOrders, ...newWOs].filter(w => w.id.startsWith(sch.equipmentId+"-"));
        const nums = existing.map(w => parseInt(w.id.split("-").pop(), 10) || 0);
        const next = nums.length > 0 ? Math.max(...nums) + 1 : 1;
        const woId = `${sch.equipmentId}-${String(next).padStart(2, "0")}`;
        const taskName = sch.task || "Scheduled service";
        const task = isInspection
          ? (state.inspectionTasks || []).find(t => t.id === sch.taskId)
          : (state.pmTasks || []).find(t => t.id === sch.taskId);
        return {
          id: woId,
          title: isInspection ? `Inspection: ${taskName}` : taskName,
          equipment: sch.equipmentId,
          status: "Open",
          priority: "Medium",
          woType: isInspection ? "Inspection" : "Service",
          created: new Date().toISOString().split("T")[0],
          due: sch.nextDueDate || new Date().toISOString().split("T")[0],
          tech: "",
          laborHours: 0,
          laborCost: 0,
          partsCost: 0,
          description: isInspection
            ? `Auto-triggered inspection (usage threshold reached): ${taskName}`
            : `Auto-triggered service (usage threshold reached): ${taskName}`,
          mechanicNotes: "",
          partsUsed: [],
          scheduleId: sch.id,
          inspectionSteps: isInspection
            ? (task?.steps || []).filter(Boolean).map(s => ({ text: s, passed: false }))
            : undefined,
          autoGenerated: true,
        };
      };

      (state.pmSchedules || []).forEach(sch => { const wo = checkAndCreateWO(sch, false); if (wo) newWOs.push(wo); });
      (state.inspectionSchedules || []).forEach(sch => { const wo = checkAndCreateWO(sch, true); if (wo) newWOs.push(wo); });

      const newNotifs = newWOs.map(wo => ({
        id: `N${Date.now()}${Math.random().toString(36).substr(2, 5)}`,
        type: "wo",
        msg: `Auto-triggered: ${wo.title} (${wo.id})`,
        time: "Just now",
        read: false,
      }));
      ...
      return {
        ...state,
        usageLogs: newLogs,
        workOrders: [...newWOs, ...state.workOrders],
...
function WorkOrders({ state, dispatch, woSettings, onWOSettings }) {
  ...
  const renderWODetail = (wo) => {
    ...
    /* Inline status change handler with re-open warning + restock parts */
    const changeStatus = (newStatus) => {
      if (newStatus === wo.status) return;
      /* Re-opening a completed WO */
      if (wo.status === "Completed" && newStatus !== "Completed") {
        const partsLines = partsUsed
          .filter(p => p.partId)
          .map(p => `• ${p.name} (${p.qty} ${p.unit || "ea"})`)
          .join("\n");
        const msg = partsLines
          ? `You are re-opening this completed work order.\n\nThe following parts will be RETURNED to inventory stock:\n\n${partsLines}\n\nIf you close this work order again, those parts will be consumed from inventory once more.\n\nContinue?`
          : "You are re-opening this completed work order. Continue?";
        if (!confirm(msg)) return;
        const restockList = partsUsed.filter(p => p.partId).map(p => ({ partId: p.partId, qty: +(p.qty||1) }));
        if (restockList.length > 0) dispatch({ type: "RESTOCK_PARTS", payload: restockList });
        const updated = { ...wo, status: newStatus, completed: null, partsRestockedOnReopen: true };
        dispatch({ type: "UPDATE_WO", payload: updated });
        setDetailWO(updated);
        return;
      }
      /* Normal status change (no re-open) */
    ...
    const changePriority = (newPriority) => {
      if (newPriority === wo.priority) return;
      const updated = { ...wo, priority: newPriority };
      dispatch({ type: "UPDATE_WO", payload: updated });
      setDetailWO(updated);
    };

    /* Complete WO - opens the confirm modal */
    const completeWO = () => {
      setShowCompleteModal(true);
    };

    /* Confirm completion (from the modal) */
    const confirmComplete = (shouldPrint) => {
      const completedDate = today();
      const updated = { ...wo, status: "Completed", completed: completedDate };
      /* Consume parts from inventory (only those linked to inventory parts) */
      const consumeList = (wo.partsUsed||[])
        .filter(p => p.partId)
        .map(p => ({ partId: p.partId, qty: +(p.qty||1) }));
      if (consumeList.length > 0) dispatch({ type: "CONSUME_PARTS", payload: consumeList });
      dispatch({ type: "UPDATE_WO", payload: updated });
      setShowCompleteModal(false);
      if (shouldPrint) printWO(updated);
      else { setModal(null); setDetailWO(null); }
    };
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
        ...
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", flexWrap:"wrap", gap:10 }}>
          <div style={{ flex:1, minWidth:240 }}>
            ...
            <div style={{ display:"flex", gap:8, marginBottom:8, flexWrap:"wrap", alignItems:"center" }}>
              <div style={{ display:"flex", alignItems:"center", gap:4 }}>
                <label style={{ fontFamily:T.sans, fontSize:11, color:T.muted, fontWeight:600 }}>Status:</label>
                <select value={wo.status} onChange={e=>changeStatus(e.target.value)} style={{ ...sel, padding:"4px 8px", fontSize:12, fontWeight:600 }}>
                  {["Open","In Progress","Awaiting Parts","On Hold","Completed"].map(s=>(<option key={s} value={s}>{s}</option>))}
                </select>
              </div>
              <div style={{ display:"flex", alignItems:"center", gap:4 }}>
                <label style={{ fontFamily:T.sans, fontSize:11, color:T.muted, fontWeight:600 }}>Priority:</label>
                <select value={wo.priority} onChange={e=>changePriority(e.target.value)} style={{ ...sel, padding:"4px 8px", fontSize:12, fontWeight:600 }}>
          ...
          <div style={{ display:"flex", gap:6, alignItems:"center" }}>
            <button onClick={()=>printWO(wo)} title="Print" style={{ background:"#fff", border:`1px solid ${T.border}`, borderRadius:6, padding:"6px 10px", cursor:"pointer", fontSize:16, lineHeight:1 }}>🖨️</button>
            {!isCompleted && (
              <button onClick={()=>setEditMode(m=>!m)} title={editMode?"Cancel edit":"Edit work order"} style={{ background:editMode?T.accentLt:"#fff", border:`1px solid ${editMode?T.accent:T.border}`, borderRadius:6, padding:"6px 10px", cursor:"pointer", fontSize:16, lineHeight:1 }}>✏️</button>
            )}
            <button onClick={()=>{ if(confirm("Delete this work order permanently?")){ del(wo.id); }}} title="Delete" style={{ background:"#fff", border:`1px solid #fca5a5`, borderRadius:6, padding:"6px 10px", cursor:"pointer", fontSize:16, lineHeight:1, color:T.red }}>✕</button>
            {!isCompleted && !editMode && <Btn small onClick={completeWO} style={{ background:T.green, borderColor:T.green, marginLeft:6 }}>✓ Complete Work Order</Btn>}
            {!isCompleted && editMode && <Btn small onClick={()=>{ dispatch({ type:"UPDATE_WO", payload:{ ...wo, ...form } }); setEditMode(false); setDetailWO({ ...wo, ...form }); }} style={{ background:T.green, borderColor:T.green, marginLeft:6 }}>Save Changes</Btn>}
          </div>
        </div>
  ...
  return (
    <div>
      ...
      {/* Complete WO confirmation modal */}
      {showCompleteModal && detailWO && (() => {
        const currentWO = state.workOrders.find(w=>w.id===detailWO.id)||detailWO;
        const inventoryParts = (currentWO.partsUsed||[]).filter(p => p.partId);
        const completedDate = today();
        const completeIt = (shouldPrint) => {
          const updated = { ...currentWO, status:"Completed", completed:completedDate };
          if(inventoryParts.length > 0) {
            dispatch({ type:"CONSUME_PARTS", payload: inventoryParts.map(p=>({partId:p.partId, qty:+(p.qty||1)})) });
          }
          dispatch({ type:"UPDATE_WO", payload:updated });
          setShowCompleteModal(false);
          if(shouldPrint) printWO(updated);
        ...
        return (
          <Modal title="Complete Work Order" onClose={()=>setShowCompleteModal(false)}>
            <div style={{ marginBottom:14 }}>
              <div style={{ fontFamily:T.sans, fontSize:14, color:T.text, lineHeight:1.5, marginBottom:12 }}>
                Are you sure all work for <b>{currentWO.id}</b> is complete?
              </div>
              {inventoryParts.length > 0 ? (
                <div style={{ background:"#fef3c7", border:"1px solid #fbbf24", borderRadius:8, padding:"12px 14px", marginBottom:10 }}>
                  <div style={{ fontFamily:T.sans, fontSize:12, fontWeight:700, color:"#92400e", marginBottom:6 }}>⚠ The following parts will be CONSUMED from inventory:</div>
                  <ul style={{ margin:0, paddingLeft:20, fontFamily:T.sans, fontSize:13, color:T.text }}>
                    {inventoryParts.map((p,i) => (
                      <li key={i} style={{ marginBottom:2 }}>{p.name} — {p.qty} {p.unit||"ea"}</li>
                    ))}
                  </ul>
                </div>
              ) : (
                <div style={{ background:T.grayLt, border:`1px solid ${T.border}`, borderRadius:8, padding:"10px 12px", fontFamily:T.sans, fontSize:12, color:T.muted }}>
                  No inventory parts are linked to this work order — nothing will be deducted from stock.
                </div>
              )}
              <div style={{ fontFamily:T.sans, fontSize:12, color:T.muted, marginTop:10, fontStyle:"italic" }}>
                If you re-open this work order later, those parts will be returned to stock.
              </div>
            </div>
            <div style={{ display:"flex", gap:8, justifyContent:"flex-end", flexWrap:"wrap" }}>
              <Btn variant="secondary" onClick={()=>setShowCompleteModal(false)}>Cancel</Btn>
              <Btn onClick={()=>completeIt(false)}>✓ Complete & Close</Btn>
              <Btn onClick={()=>completeIt(true)} style={{ background:T.accent }}>🖨 Complete & Print</Btn>
            </div>
          </Modal>
        );
      })()}
...
function Inspections({ state, dispatch }) {
  const [modal, setModal]             = useState(null); /* null | "schedule" | "manual" | "editSch" */
  const [taskModal, setTaskModal]     = useState(false);
  const [editTaskId, setEditTaskId]   = useState(null);
  ...
  const saveSchedule = () => {
    ...
    const nextUsage = schForm.triggerType!=="time"  ? (+(schForm.lastDoneUsage||0))+(+(schForm.usageInterval||0)) : "";
    if(editSchId) {
      dispatch({ type:"UPDATE_INSP_SCHEDULE", payload:{
        ...schForm, id:editSchId,
        nextDueDate:nextDate, nextDueUsage:nextUsage,
      }});
    } else {
    ...
    setModal(null); setEditSchId(null);
    setSchForm({ equipmentId:"", taskId:"", task:"", triggerType:"time", timeInterval:"", timeUnit:"months", usageInterval:"", usageType:"hours", lastDoneDate:today(), lastDoneUsage:"" });
  };
  ...
  const renderSection = (title, items, color) => items.length===0 ? null : (
    <Card style={{ marginBottom:14, padding:0, overflow:"hidden" }}>
      ...
      <table style={{ width:"100%", borderCollapse:"collapse", fontSize:13, fontFamily:T.sans }}>
        ...
        <tbody>
          {items.map(s=>(
            <tr key={s.id} onClick={()=>openEditSchedule(s)} style={{ borderBottom:`1px solid ${T.border}`, cursor:"pointer" }}
              onMouseEnter={e=>e.currentTarget.style.background=T.accentLt}
              onMouseLeave={e=>e.currentTarget.style.background=""}>
              <td style={{ padding:"9px 12px", color:T.accent, fontWeight:500, textDecoration:"underline" }}>{s.eqName}</td>
              ...
              <td style={{ padding:"9px 12px", textAlign:"right" }} onClick={e=>e.stopPropagation()}>
                <Btn small variant="secondary" onClick={()=>openEditSchedule(s)} style={{ marginRight:4 }}>✏️ Edit</Btn>
  ...
  return (
    <div>
      ...
      {/* Assign Task to Equipment modal — also used for edit */}
      {modal==="schedule" && (
        <Modal title={editSchId?"Edit Inspection Schedule":"Assign Inspection Task to Equipment"} onClose={()=>{ setModal(null); setEditSchId(null); setSchForm({ equipmentId:"", taskId:"", task:"", triggerType:"time", timeInterval:"", timeUnit:"months", usageInterval:"", usageType:"hours", lastDoneDate:today(), lastDoneUsage:"" }); }}>
          <p style={{ margin:"0 0 14px", fontFamily:T.sans, fontSize:13, color:T.subtext }}>
          ...
          <div style={{ display:"flex", gap:8, justifyContent:"flex-end", marginTop:14 }}>
            <Btn variant="secondary" onClick={()=>{ setModal(null); setEditSchId(null); }}>Cancel</Btn>
            <Btn onClick={saveSchedule}>{editSchId?"Update Schedule":"Assign Inspection"}</Btn>
          </div>
        </Modal>
      )}
...
function App() {
  ...
  /* Initial tab from URL hash, default dashboard */
  const initialTab = (() => {
  ...
  /* Wrapped setTab that pushes to browser history so back button works */
  const setTab = (newTab) => {
  ...
  useEffect(() => {
    ...
    /* On initial mount, replace history state so the first state has tab info */
